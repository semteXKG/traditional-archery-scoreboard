...
public class TouchListView extends ListView {
  ...
  public TouchListView(final Context context, final AttributeSet attrs, final int defStyle) {
    ...
    if (attrs != null) {
      final TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.TouchListView, 0, 0);
  ...
  @Override
  final public void addHeaderView(final View v, final Object data, final boolean isSelectable) {
    throw new RuntimeException("Headers are not supported with TouchListView");
  }


  @Override
  final public void addHeaderView(final View v) {
    throw new RuntimeException("Headers are not supported with TouchListView");
  }


  @Override
  final public void addFooterView(final View v, final Object data, final boolean isSelectable) {
  ...
  @Override
  final public void addFooterView(final View v) {
    if (mRemoveMode == SLIDE_LEFT || mRemoveMode == SLIDE_RIGHT) {
      throw new RuntimeException("Footers are not supported with TouchListView in conjunction with remove_mode");
    }
  }
      ...
      switch (ev.getAction()) {
        case MotionEvent.ACTION_DOWN:
          ...
          if (isDraggableRow(item)) {
            ...
            final View dragger = item.findViewById(grabberId);
            final Rect r = mTempRect;
  ...
  protected boolean isDraggableRow(final View view) {
  ...
  private int getItemForPosition(final int y) {
    final int adjustedy = y - mDragPoint - mItemHeightNormal / 2;
   ...
   * listitem's size normal, but make the item invisible. Otherwise, if the dragged listitem is still on screen, make it
   * as small as possible and expand the item below the insert point. If the dragged item is not on screen, only expand
   * the item below the current insertpoint.
   */
